...
class DistributedServer(object):
    def __init__(self, global_model, host, port, nodes, cid):
        ...
        self.NUM_CLIENTS_CONTACTED_PER_ROUND = self.MIN_NUM_WORKERS
        self.F = (self.NUM_CLIENTS_CONTACTED_PER_ROUND-1)//2
        self.ROUNDS_BETWEEN_VALIDATIONS = 5
        self.MAX_DEPTH_OF_TREE = 2
    ...
    def register_handles(self):
        ...
        def handle_client_ready(data):
            ...
            if len(self.ready_client_sids) >= self.NUM_CLIENTS_CONTACTED_PER_ROUND and self.current_round == -1 and self.stop_and_eval_flag == False:
        ...
        def handle_client_vote(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                if temp_result and len(self.current_round_client_updates) >= self.NUM_CLIENTS_CONTACTED_PER_ROUND: 
                    voted_weights = []
                    voted_train_loss = []
                    voted_train_size = []
                    ...
                    if len(voted_weights) == 0:
                        print("[server%s] Client voting failure, the test threshold_accuracy setting is too strict..."%self.serverID)  # all voted models are unactive
                        self.stop_and_eval()
                        return
        ...
        def handle_client_local_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                Node_List = []
                if self.global_model.Mtree.depth() > self.MAX_DEPTH_OF_TREE:
                ...
                if len(self.temp) >= self.NUM_CLIENTS_CONTACTED_PER_ROUND:   # TODO: Is this sequential?
        ...
        def handle_client_eval(data):
            ...
            # tolerate 30% unresponsive clients, speed up evaluation
            if len(self.eval_client_updates) >= int(self.NUM_CLIENTS_CONTACTED_PER_ROUND * .8):
        ...
        def replyTrackerHandler(tag):
            if self.replyTracker.complete(tag):
                ...
                if self.replyTracker.ResponseCt[tag] > self.F:
    ...
    def train_next_round(self):
        ...
        print("\n\n[server%s] ### Round "%self.serverID, self.current_round, "###")
        client_sids_selected = random.sample(list(self.ready_client_sids), self.NUM_CLIENTS_CONTACTED_PER_ROUND)
        print("[server%s] request updates from"%self.serverID, client_sids_selected)
    ...
    def stop_and_eval(self):
        ...
        self.eval_client_updates = []
        #print("[server%s] converges review list: "%self.serverID,self.converges_review)
        #print("[server%s] aggregate train accuracy list: "%self.serverID,self.agg_train_accuracy)
    ...
    def save_accuracy_to_file(self):
        print("[server%s] Current converge status: "%self.serverID,self.converges_review)
        print("[server%s] Current accuracy change: "%self.serverID,self.agg_train_accuracy)
        print("[Evaluation] [server%s] Global model validate accuracy change: "%self.serverID,self.aggr_valid_accuracy)
    ...
    def clean_up_tree(self):
        ...
        for x in self.global_model.Mtree.nodes.items():
            ...
            if self.global_model.Mtree.depth(x[-1]) == self.global_model.Mtree.depth() - self.MAX_DEPTH_OF_TREE:
                if not x[-1].is_leaf():
