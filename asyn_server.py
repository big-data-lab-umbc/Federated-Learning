...
class FLServer(object):
    ...
    def register_handles(self):
        ...
        def handle_client_ready(data):
            print("client%s ready for training"%str(data['client_ID']), request.sid, data['train_size'])
        ...
        def handle_client_vote(data):
            ...
            if data['sequence_number'] >= max(self.sequence_number_list) - Asynchronous_Round_Threshold*len(self.ready_client_sids):
                reviews = []
                for x in [data]: 
                ...
                if temp_result and self.stop_and_eval_flag == False: 
                    ...
                    # if all voted models are inactive, stop training and review the client threshold_accuracy
                    if inactive_model == len(self.ready_client_sids):
                        print("Client voting failure, the test threshold_accuracy setting is too strict...")  
                        self.stop_and_eval()
                        return
                    ...
                    if self.global_model.Mtree.depth() > MAX_NUM_ROUNDS:
                        self.stop_and_eval()
                        return
                    else:
        ...
        def handle_client_local_update(data):
            ...
            if data['sequence_number'] >= max(self.sequence_number_list) - Asynchronous_Round_Threshold*len(self.ready_client_sids) and self.stop_and_eval_flag == False:
        ...
        def handle_client_eval(data):
            ...
            if len(self.eval_client_updates) > MIN_NUM_WORKERS * .8:
        ...
        def replyTrackerHandler(tag):
            if replyTracker.complete(tag):
                ...
                if replyTracker.ResponseCt[tag] > F:  # TODO: this is crash failure
        ...
        def train_next_round(rid):
            ...
            print("\n\n### For model sequence number ", self.sequence_number, "###")
            print("request updates from", rid)
    ...
    def clean_up_tree(self):
        ...
        if current_depth <= MAX_DEPTH_OF_TREE:
            return None
        out_of_date_node = []
        Node_List = []
        for x in self.global_model.Mtree.nodes.items():
            if x[-1].data.if_active == '':
                Node_List.append(x[-1])
            if x[-1].is_leaf() and current_depth - self.global_model.Mtree.depth(x[-1]) >= MAX_DEPTH_OF_TREE and x[-1].data.if_active == None:
...
if __name__ == '__main__':
    ...
    F = (MIN_NUM_WORKERS-1)//2
    REQUEST_NUMBER_BETWEEN_VALIDATIONS = MIN_NUM_WORKERS*2

    Asynchronous_Round_Threshold = 2
    MAX_DEPTH_OF_TREE = 5
    MAX_NUM_ROUNDS = 1
